...
def selector():
	op=int(input("Digite para escolher: \n1 - Webcam \n2 - video\n"))
	if(op==1):
...
out=cv2.VideoWriter("teste.avi",fourcc,30,(854,480))
...
while True:
	...
	# Resize the frame 
	frame = imutils.resize(frame, width=426, height=240)
	...
	for (i, rect) in enumerate(rects):
		...
		# Put a number 
		cv2.putText(frame, "Driver", (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
		...
		if EAR < EAR_THRESHOLD: 
			...
			if FRAME_COUNT >= CONSECUTIVE_FRAMES:
				pass 
				#T = Thread(target=alerta,args=('warning',))
				#T.start()
				#alerta('warning')
			FRAME_COUNT = 0
